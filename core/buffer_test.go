...
func TestNewBuffer(t *testing.T) {
	bf, err := NewBuffer(1024, WithMetrics(_const.PrometheusCollector))
...
func BenchmarkBuffer(b *testing.B) {
	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			buffer, _ := NewBuffer(1024, WithMetrics(_const.PrometheusCollector))
			defer buffer.Close()
...
func BenchmarkBuffer_Write(b *testing.B) {
	...
	for _, size := range sizes {
		b.Run(formatSize(size), func(b *testing.B) {
			buf, _ := NewBuffer(1024, WithMetrics(_const.PrometheusCollector))
			defer buf.Close()
